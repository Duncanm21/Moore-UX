...
const ProjectCardGlide = ({
  className = "",
  glidanceDocumentary,
  screenArea,
  scanLines,
  plantGroup,
  videoProducerUXDesigner,
  rectangle11,
  rectangle111,
  rectangle112,
  projectCardGlideBackgroundImage,
  frameBoxBackground,
  frameBoxBorder,
}) => {
  const projectCardGlideStyle = useMemo(() => {
    return {
      backgroundImage: projectCardGlideBackgroundImage,
    };
  }, [projectCardGlideBackgroundImage]);

  const frameBoxStyle = useMemo(() => {
    return {
      background: frameBoxBackground,
      border: frameBoxBorder,
    };
  }, [frameBoxBackground, frameBoxBorder]);

  return (
    <Box
      className={`h-[620px] w-[390px] rounded-3xs overflow-hidden shrink-0 flex flex-row items-center justify-center flex-wrap content-center p-6 box-border gap-6 bg-[url('/public/projectcardglide@3x.png')] bg-cover bg-no-repeat bg-[top] text-center text-11xl text-orange-200 font-vcr-osd-mono ${className}`}
      style={projectCardGlideStyle}
    >
      <Box className="w-[305px] flex flex-row items-start justify-center flex-wrap content-start gap-2.5">
        ...
        <Box className="h-[250px] rounded-8xs bg-darkslategray overflow-hidden flex flex-col items-start justify-start p-3xs box-border relative gap-2.5">
          ...
          <img
            className="w-[300px] absolute !m-[0] top-[30px] left-[20px] rounded-8xs h-[180px] object-cover z-[1]"
            alt=""
            src={screenArea}
          />
          <img
      ...
      <Box className="h-[100px] w-[340px] rounded-3xs bg-gray-300 flex flex-row items-center justify-center p-3xs box-border gap-2.5 text-5xl">
        <Box
          className="self-stretch rounded-8xs [background:linear-gradient(rgba(22,_26,_29,_0.85),_rgba(22,_26,_29,_0.85)),_#46579f] border-mediumvioletred border-solid border-[2px] flex flex-row items-center justify-start p-3xs"
          style={frameBoxStyle}
        >
          <img
...
ProjectCardGlide.propTypes = {
  className: PropTypes.string,
  glidanceDocumentary: PropTypes.string,
  screenArea: PropTypes.string,
  scanLines: PropTypes.string,
  plantGroup: PropTypes.string,
  videoProducerUXDesigner: PropTypes.string,
  rectangle11: PropTypes.string,
  rectangle111: PropTypes.string,
  rectangle112: PropTypes.string,

  /** Style props */
  projectCardGlideBackgroundImage: PropTypes.string,
  frameBoxBackground: PropTypes.string,
  frameBoxBorder: PropTypes.string,
};

export default ProjectCardGlide;
